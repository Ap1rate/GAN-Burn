...
class SwinUnetGenerator(nn.Module):
    """U-Net generator with Swin Transformer at the bottleneck."""

    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        super(SwinUnetGenerator, self).__init__()
        # 1. encoder (downsample, same as U-Net)
        # 2. bottleneck with swin transformer
        # 3. decoder (upsample + skip connection)

        # TODO: 这里需要你决定是否用轻量化 SwinBlock
        # 可以从 huggingface timm 里调用 SwinTiny，或者写一个简化版

    def forward(self, x):
        # forward path with skip connections
        return out
...
def define_G(input_nc, output_nc, ngf, netG, norm="batch", use_dropout=False, init_type="normal", init_gain=0.02):
    ...
    if netG == "resnet_9blocks":
        ...
        net = SwinUnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    elif netG == "unet_128":
